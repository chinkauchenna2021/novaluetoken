...
const CreateFreeToPlay = () => {
  ...
  const recieveDoublediceFaucet = async (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    let send_address = process.env.NEXT_PUBLIC_WALLET_ADDRESS;
    ...
    let contract_address = "0x7263CFC20b4C2e4ADa5b59F7e907A80BB8CDAb77";
    ...
    if (contract_address) {
      // general token send
      let contract = new ethers.Contract(contract_address, DummyUSDTethers.abi, walletSigners);
      // How many tokens?
      ...
      try {
        ...
        if (transferResult) {
          alert(`you have been sent 1000 USDT`);
        }
      } catch (e) {
        alert(`your wallet has been gray listed. you can only recieve USDT after 24 hours`);
      }
    } // ether send
    else {
      const tx = {
        ...
        nonce: ethersProvider.getTransactionCount(send_address, "latest"),
        gasLimit: ethers.utils.hexlify(gas_limit), // 100000
        gasPrice: gas_price,
      };
      try {
        walletSigners.sendTransaction(tx).then((transaction) => {
          alert("Send finished!");
        });
      } catch (error) {
  ...
  const addToWallet = async (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const tokenAddress = "0x7263CFC20b4C2e4ADa5b59F7e907A80BB8CDAb77";
  ...
  return (
    <SC.Holder>
      ...
      <div style={{ width: "100%", display: "flex", justifyContent: "flex-end" }}>
        {" "}
        <SC.Button type="submit" onClick={() => connectToMetamask()}>
          🦊 connect to metamask
        </SC.Button>
      </div>
      <SC.Button type="submit" onClick={(e) => addToWallet(e)}>
        🦊 Add USDT to metamask
      </SC.Button>
      <SC.Button type="submit" onClick={(e) => recieveDoublediceFaucet(e)}>
        GET 💰💰💰 1000 USDT{" "}
      </SC.Button>
      {/* <SC.Button type="submit" onClick={enterToPlayPage}>
        ⏪⏪⏪ Enter Free to Play Page{" "}
      </SC.Button> */}
    </SC.Holder>
  );
};
