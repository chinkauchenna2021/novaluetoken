...
export async function deployDummyUSDTether(deployer: SignerWithAddress): Promise<DummyUSDTether> {
  const contract = await new DummyUSDTether__factory(deployer).deploy();
  process.stdout.write(`Deploying WBTC contract to: ${contract.address}...\n`);
  process.stdout.write(`Sent transaction: ${contract.deployTransaction.hash}\n`);
  await contract.deployed();
  return contract;
